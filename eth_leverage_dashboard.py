...
for s_ltv in second_loop_lvts:
    for f_ltv in first_loop_lvts:
        ...
        loop1_usdc = round((eth_stack * eth_price) * (f_ltv / 100), 2)
        loop2_usdc = round((eth_stack * eth_price + loop1_usdc) * (s_ltv / 100), -2)
...
# Rank top 10 by Total ETH
df = pd.DataFrame(heatmap_data)
df_sorted = df.sort_values(by="Total ETH", ascending=False).reset_index(drop=True)
df_sorted["Rank"] = df_sorted.index + 1
df = df.merge(df_sorted[["First LTV", "Second LTV", "Rank"]], on=["First LTV", "Second LTV"])  

# Create label for heatmap
labels = df.apply(lambda row: f"{row['Final Health Score']:.2f}\n${int(row['Loop 2 USDC'])}\n↓{int(row['Liq % Drop'])}% @ ${row['Liq Price']}\n{row['Total ETH']:.2f} ETH (+{int(row['% Gain'])}%)\n#{row['Rank']}", axis=1)
df["Label"] = labels

# Pivot data
pivot_scores = df.pivot(index="Second LTV", columns="First LTV", values="Final Health Score")
pivot_labels = df.pivot(index="Second LTV", columns="First LTV", values="Label")

# Display heatmap
fig, ax = plt.subplots(figsize=(14, 10))
sns.heatmap(pivot_scores, annot=pivot_labels, fmt="", cmap="RdYlGn", cbar_kws={'label': 'Final Health Score'}, ax=ax)
